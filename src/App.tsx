...
export default function App(){
  ...
  return <div style={{maxWidth:420,margin:'0 auto',padding:16,fontFamily:'sans-serif'}}>
    ...
    <div style={{marginTop:20}}>{exps.map(e=><div key={e.id} style={{display:'flex',justifyContent:'space-between',padding:'12px 0',borderBottom:'1px solid #eee'}}><div><b>{e.title}</b><br/><small>{e.cat}</small></div><b>₹{e.amount}</b></div>)}</div>
  </div>
}
